...
class Keyframe:
    ...
    def generate_keyframes(self, _id: str) -> None:
        ...
        try:
            ...
            output_dir = f'screenshot/{video_id}'
            if os.path.exists(output_dir) and os.listdir(output_dir):
                logging.info(f'{video_id}: Screenshots already exist, skipping processing')
            else:
                ...
                if fps == 0:
                    logging.error(f'{video_id}: FPS value is zero, cannot calculate total duration')
                ...
                for time in timestamps:
                    ...
                    if ret:
                        ...
                        print(f'{video_id}: Failed. Could not read frame at {int(time/1000)} seconds')
                        logging.error(f'{video_id}: Failed. Could not read frame at {int(time/1000)} seconds')

                cap.release()

                os.remove(temp_video_path)
                print(f'{video_id}: Keyframes generated successfully')
                logging.info(f'{video_id}: Keyframes generated successfully')
